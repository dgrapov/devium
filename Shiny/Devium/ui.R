...
shinyUI(
  ...
  pageWithSidebar(
    ...
    mainPanel(
      conditionalPanel(condition = "input.datasets != ''", # data View table
        conditionalPanel(condition = "input.tool == 'dataview'", 
          tabsetPanel(id = "datatabs",
            ...
            tabPanel("Visualize", plotOutput("visualize",width = 850, height = 650)),#height = "100%"
            tabPanel("About", includeMarkdown("about.md")),
			tabPanel("Debug", verbatimTextOutput("debug"))
            # tabPanel("About", includeRmd("about_formulas.Rmd"))
          )
        ),
        conditionalPanel(condition = "input.tool != 'dataview'", # conditional UI when not in data view
